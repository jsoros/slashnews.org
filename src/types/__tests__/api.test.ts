...
describe("API Validation Functions", () => {
  describe("validateStoryId", () => {
    ...
    it("should throw an error for non-number inputs", () => {
      expect(() => validateStoryId("1")).toThrowError(
        "Story ID must be a positive integer",
      );
      expect(() => validateStoryId(null)).toThrowError(
        "Story ID must be a positive integer",
      );
      expect(() => validateStoryId(undefined)).toThrowError(
        "Story ID must be a positive integer",
      );
      expect(() => validateStoryId({})).toThrowError(
        "Story ID must be a positive integer",
      );
      expect(() => validateStoryId([])).toThrowError(
        "Story ID must be a positive integer",
      );
      expect(() => validateStoryId(NaN)).toThrowError(
        "Story ID must be a positive integer",
      );
    });

    it("should throw an error for numbers <= 0", () => {
      expect(() => validateStoryId(0)).toThrowError(
        "Story ID must be a positive integer",
      );
      expect(() => validateStoryId(-1)).toThrowError(
        "Story ID must be a positive integer",
      );
      expect(() => validateStoryId(-100)).toThrowError(
        "Story ID must be a positive integer",
      );
    });

    it("should throw an error for non-integer numbers", () => {
      expect(() => validateStoryId(1.5)).toThrowError(
        "Story ID must be a positive integer",
      );
      expect(() => validateStoryId(3.14159)).toThrowError(
        "Story ID must be a positive integer",
      );
    });
  });
  ...
  describe("validateUrl", () => {
    ...
    it("should throw an error for non-string or empty inputs", () => {
      expect(() => validateUrl(null)).toThrowError(
        "URL must be a non-empty string",
      );
      expect(() => validateUrl(undefined)).toThrowError(
        "URL must be a non-empty string",
      );
      expect(() => validateUrl(123)).toThrowError(
        "URL must be a non-empty string",
      );
    ...
    it("should throw an error for non-http/https protocols", () => {
      ...
      expect(() => validateUrl('javascript:alert("xss")')).toThrowError(
        "URL must use HTTP or HTTPS protocol",
      );
      expect(() => validateUrl("file:///etc/passwd")).toThrowError(
        "URL must use HTTP or HTTPS protocol",
      );
      expect(() => validateUrl("mailto:test@example.com")).toThrowError(
        "URL must use HTTP or HTTPS protocol",
      );
    });
  });
  ...
  describe("validateHackerNewsItem", () => {
    ...
    it("should throw an error if item is not an object", () => {
      expect(() => validateHackerNewsItem(null)).toThrowError(
        "Item must be an object",
      );
      expect(() => validateHackerNewsItem(undefined)).toThrowError(
        "Item must be an object",
      );
      expect(() => validateHackerNewsItem("string")).toThrowError(
        "Item must be an object",
      );
      expect(() => validateHackerNewsItem(123)).toThrowError(
        "Item must be an object",
      );
    });
    ...
    it("should throw an error if item has missing or invalid ID", () => {
      ...
      expect(() => validateHackerNewsItem(missingId)).toThrowError(
        "Item must have a valid ID",
      );

      const invalidId = { id: 0, type: "story", time: 123, title: "Title" };
      expect(() => validateHackerNewsItem(invalidId)).toThrowError(
        "Item must have a valid ID",
      );

      const stringId = { id: "1", type: "story", time: 123, title: "Title" };
      expect(() => validateHackerNewsItem(stringId)).toThrowError(
        "Item must have a valid ID",
      );
    });
    ...
    it("should throw an error if item has missing or invalid type", () => {
      ...
      expect(() => validateHackerNewsItem(missingType)).toThrowError(
        "Item must have a valid type",
      );

      const invalidType = { id: 1, type: "unknown", time: 123, title: "Title" };
      expect(() => validateHackerNewsItem(invalidType)).toThrowError(
        "Item must have a valid type",
      );
    });
    ...
    it("should throw an error if item has missing or invalid timestamp", () => {
      ...
      expect(() => validateHackerNewsItem(missingTime)).toThrowError(
        "Item must have a valid timestamp",
      );

      const invalidTime = { id: 1, type: "story", time: 0, title: "Title" };
      expect(() => validateHackerNewsItem(invalidTime)).toThrowError(
        "Item must have a valid timestamp",
      );

      const stringTime = { id: 1, type: "story", time: "123", title: "Title" };
      expect(() => validateHackerNewsItem(stringTime)).toThrowError(
        "Item must have a valid timestamp",
      );
    });
    ...
    it("should throw an error if a story is missing a title", () => {
      ...
      expect(() => validateHackerNewsItem(missingTitle)).toThrowError(
    ...
    it("should throw an error if a comment is missing text or parent", () => {
      ...
      expect(() => validateHackerNewsItem(missingText)).toThrowError(
        "Comment must have text and parent",
      );
      ...
      expect(() => validateHackerNewsItem(missingParent)).toThrowError(
        "Comment must have text and parent",
      );
    });
  });
